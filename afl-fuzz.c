...
static void usleep(u64 usec)
{

  HANDLE timer = NULL;
  LARGE_INTEGER delta;

  delta.QuadPart = -(10 * usec);

  timer = CreateWaitableTimer(NULL, TRUE, NULL);
  if (timer == NULL) {
    FATAL("usleep: failed to CreateWaitableTimer");
  }
  if (!SetWaitableTimer(timer, &delta, 0, NULL, NULL, 0)) {
    FATAL("usleep: failed to SetWaitableTimer");
  }
